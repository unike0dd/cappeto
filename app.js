...
$('previewOwnerButton').addEventListener('click',async()=>{state.staff=window.CAPPETO_PREVIEW_SESSION.startPreviewSession(sessionStorage,{id:'preview-owner',role:'owner',displayName:t('previewOwnerName'),preview:true});state.csrf='';showStaffControls(true);await loadCatalog();enterApp();switchView('manage')});
