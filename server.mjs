...
const server=createServer(async(req,res)=>{try{
  ...
  if(url.pathname==='/api/products'&&req.method==='GET'){
    ...
    const products=data.products.map(({procurementCostCents,purchaseTaxRate,purchaseDeliveryCents,procurementQuantity,returned,damaged,sold,purchaseDate,...product})=>product);
  ...
  if(url.pathname==='/api/products'&&req.method==='POST'){
    ...
    const name=clean(body.name,80),category=clean(body.category,40),description=clean(body.description,140),priceCents=Math.round(Number(body.price)*100),procurementCostCents=Math.round(Number(body.procurementCost||0)*100),purchaseTaxRate=Number(body.purchaseTaxRate||0),purchaseDeliveryCents=Math.round(Number(body.purchaseDeliveryCost||0)*100),vatRate=Number(body.vatRate),stock=Number(body.stock),purchaseDate=clean(body.purchaseDate,10);
    if(!name||!category||!description||!Number.isInteger(priceCents)||priceCents<1||priceCents>999900||!Number.isInteger(procurementCostCents)||procurementCostCents<0||procurementCostCents>999900||!Number.isFinite(purchaseTaxRate)||purchaseTaxRate<0||purchaseTaxRate>100||!Number.isInteger(purchaseDeliveryCents)||purchaseDeliveryCents<0||purchaseDeliveryCents>999900||!Number.isFinite(vatRate)||vatRate<0||vatRate>100||!Number.isInteger(stock)||stock<0||stock>9999||!/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate))return json(res,400,{error:'Check the product details and try again.'});
    ...
    const id=`${name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,48)||'product'}-${randomBytes(3).toString('hex')}`;await writeFile(join(uploadsDir,`${id}.webp`),image,{flag:'wx'});const product={id,name,category,description,priceCents,procurementCostCents,purchaseTaxRate,purchaseDeliveryCents,procurementQuantity:stock,vatRate,stock,purchaseDate,returned:0,damaged:0,sold:0,imageUrl:`/uploads/${id}.webp`};data.products.push(product);await saveCatalog(data);return json(res,201,{product})
  }
  if(url.pathname.startsWith('/api/products/')&&req.method==='PUT'){
    ...
    const name=clean(body.name,80),category=clean(body.category,40),description=clean(body.description,140),priceCents=Math.round(Number(body.price)*100),procurementCostCents=Math.round(Number(body.procurementCost||0)*100),purchaseTaxRate=Number(body.purchaseTaxRate||0),purchaseDeliveryCents=Math.round(Number(body.purchaseDeliveryCost||0)*100),vatRate=Number(body.vatRate),stock=Number(body.stock),purchaseDate=clean(body.purchaseDate,10);
    if(!name||!category||!description||!Number.isInteger(priceCents)||priceCents<1||priceCents>999900||!Number.isInteger(procurementCostCents)||procurementCostCents<0||procurementCostCents>999900||!Number.isFinite(purchaseTaxRate)||purchaseTaxRate<0||purchaseTaxRate>100||!Number.isInteger(purchaseDeliveryCents)||purchaseDeliveryCents<0||purchaseDeliveryCents>999900||!Number.isFinite(vatRate)||vatRate<0||vatRate>100||!Number.isInteger(stock)||stock<0||stock>9999||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(purchaseDate))return json(res,400,{error:'Check the product details and try again.'});
    ...
    Object.assign(product,{name,category,description,priceCents,procurementCostCents,purchaseTaxRate,purchaseDeliveryCents,procurementQuantity:stock,vatRate,stock,purchaseDate,imageUrl});await saveCatalog(data);return json(res,200,{product})
  }
