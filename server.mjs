...
const server=createServer(async(req,res)=>{try{
  ...
  if(url.pathname==='/api/products'&&req.method==='GET'){
    ...
    const products=data.products.map(({procurementCostCents,returned,damaged,sold,purchaseDate,...product})=>product);
  ...
  if(url.pathname==='/api/products'&&req.method==='POST'){
    ...
    const name=clean(body.name,80),category=clean(body.category,40),description=clean(body.description,140),priceCents=Math.round(Number(body.price)*100),procurementCostCents=Math.round(Number(body.procurementCost)*100),vatRate=Number(body.vatRate),stock=Number(body.stock),purchaseDate=clean(body.purchaseDate,10);
    if(!name||!category||!description||!Number.isInteger(priceCents)||priceCents<1||priceCents>999900||!Number.isInteger(procurementCostCents)||procurementCostCents<1||procurementCostCents>999900||!Number.isFinite(vatRate)||vatRate<0||vatRate>100||!Number.isInteger(stock)||stock<0||stock>9999||!/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate))return json(res,400,{error:'Check the product details and try again.'});
  ...
  if(url.pathname.startsWith('/api/products/')&&req.method==='PUT'){
    ...
    const name=clean(body.name,80),category=clean(body.category,40),description=clean(body.description,140),priceCents=Math.round(Number(body.price)*100),procurementCostCents=Math.round(Number(body.procurementCost)*100),vatRate=Number(body.vatRate),stock=Number(body.stock),purchaseDate=clean(body.purchaseDate,10);
    if(!name||!category||!description||!Number.isInteger(priceCents)||priceCents<1||priceCents>999900||!Number.isInteger(procurementCostCents)||procurementCostCents<1||procurementCostCents>999900||!Number.isFinite(vatRate)||vatRate<0||vatRate>100||!Number.isInteger(stock)||stock<0||stock>9999||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(purchaseDate))return json(res,400,{error:'Check the product details and try again.'});
    ...
    Object.assign(product,{name,category,description,priceCents,procurementCostCents,vatRate,stock,purchaseDate,imageUrl});await saveCatalog(data);return json(res,200,{product})
  }
