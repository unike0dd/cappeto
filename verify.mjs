...
for(const marker of ['authForm','productImage','productVat','productPurchaseDate','manageSearch','productSubmitButton','subtotal','vat','total','previousProduct','nextProduct','carouselPosition'])if(!html.includes(marker))throw new Error(`Missing UI marker: ${marker}`);
...
for(const marker of ['editingProductId','Update product','updateProductAction','data-edit'])if(!script.includes(marker))throw new Error(`Missing product update behavior: ${marker}`);
