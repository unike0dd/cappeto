...
for(const marker of ['authForm','productImage','productVat','productPurchaseDate','manageSearch','productSubmitButton','clearProductButton','capacityMessage','subtotal','vat','total','previousProduct','nextProduct','carouselPosition'])if(!html.includes(marker))throw new Error(`Missing UI marker: ${marker}`);
...
for(const marker of ['editingProductId','Update product','updateProductAction','clearProductForm','data-edit','20-product limit has been reached'])if(!script.includes(marker)&&!html.includes(marker))throw new Error(`Missing product update behavior: ${marker}`);
